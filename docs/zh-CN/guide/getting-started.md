...
在 [Cloudflare 仪表盘](https://dash.cloudflare.com/) 中创建 Sink 所需的服务。稍后将其**绑定**到项目：绑定即按固定名称将数据库或存储连接至 Sink。

| 绑定名称    | Cloudflare 产品                  | 是否必需 | 说明                |
| ----------- | -------------------------------- | -------- | ------------------- |
| `DB`        | **D1**（数据库）                 | 必需     | 保存链接            |
| `KV`        | **KV**（键值存储）               | 必需     | 加速短链跳转        |
| `ANALYTICS` | **Analytics Engine**（分析引擎） | 推荐     | 访问统计与日志      |
| `R2`        | **R2**（对象存储）               | 可选     | 备份与社交分享图片  |
...
这是**仪表盘登录密码**，也是 API 工具使用的密码。请用足够长的随机字符串（至少 8 个字符，不含空格等空白字符），并保持稳定 — 改了之后所有人都会退出登录。
